...
let publicCheckoutMap=new Map();
function renderOrders(privateSnap){
  const rows=$("storeOrderRows"); if(!rows)return;
  if(privateSnap.empty){rows.innerHTML='<tr><td colspan="7">No orders yet.</td></tr>';return;}
  rows.innerHTML=privateSnap.docs.slice().sort((a,b)=>(b.data().createdAt?.seconds||0)-(a.data().createdAt?.seconds||0)).map(d=>{const o={...d.data(),...(publicCheckoutMap.get(d.id)||{})};const date=o.createdAt?.toDate?o.createdAt.toDate().toLocaleString():"—";return `<tr><td><code>${d.id.slice(0,10)}</code></td><td>${escapeHtml(o.customerName||"—")}<br><small>${escapeHtml(o.customerEmail||"")}</small></td><td>${escapeHtml(o.productName||"—")}</td><td>${escapeHtml(o.currency||"PHP")} ${Number(o.amount||0).toFixed(2)}</td><td><select class="order-status-select" data-order-status="${d.id}"><option value="pending" ${o.status==="pending"?"selected":""}>Pending</option><option value="verifying" ${o.status==="verifying"?"selected":""}>Verifying</option><option value="paid" ${o.status==="paid"?"selected":""}>Paid / Unlocked</option><option value="failed" ${o.status==="failed"?"selected":""}>Failed</option><option value="cancelled" ${o.status==="cancelled"?"selected":""}>Cancelled</option></select></td><td>${o.receiptUrl?`<a href="${escapeHtml(o.receiptUrl)}" target="_blank" rel="noopener">View proof</a>`:"—"}<br><small>${escapeHtml(date)}</small></td><td><button class="text-btn" data-open-order="${d.id}">Open</button></td></tr>`}).join("");
}
function loadOrders(){
  if(!auth.currentUser)return; const rows=$("storeOrderRows"); if(!rows)return;
  onSnapshot(collection(db,"storeCheckout"),snap=>{publicCheckoutMap.clear();snap.forEach(d=>publicCheckoutMap.set(d.id,{id:d.id,...d.data()}));});
  onSnapshot(collection(db,"storeOrders"),renderOrders,err=>{console.error("Could not load orders:",err);rows.innerHTML=`<tr><td colspan="7">Could not load orders: ${escapeHtml(err?.code||err?.message||"Firestore error")}</td></tr>`;});
}
async function updateOrderStatus(id,status){const orderRef=doc(db,"storeOrders",id);const publicRef=doc(db,"storeCheckout",id);const before=await getDoc(orderRef);const prev=before.data()||{};await updateDoc(orderRef,{status,updatedAt:serverTimestamp()});try{await updateDoc(publicRef,{status,updatedAt:serverTimestamp()});}catch(e){console.warn("Public checkout status update failed",e);}if(status==="paid"&&prev.status!=="paid"&&prev.productId){const pRef=doc(db,"storeProducts",prev.productId);await runTransaction(db,async tx=>{const ps=await tx.get(pRef);if(!ps.exists())return;const current=Math.max(0,Number(ps.data().stock||0));tx.update(pRef,{stock:Math.max(0,current-1),updatedAt:serverTimestamp()});});sendPaymentEmail({action:"paymentVerified",customerEmail:prev.customerEmail,customerName:prev.customerName,productName:prev.productName,amount:prev.amount,currency:prev.currency,orderId:id,unlockUrl:prev.unlockUrl||"",accessUsername:prev.accessUsername||"",accessPassword:prev.accessPassword||""});} else if(status==="failed"&&prev.status!=="failed"){sendPaymentEmail({action:"paymentFailed",customerEmail:prev.customerEmail,customerName:prev.customerName,productName:prev.productName,amount:prev.amount,currency:prev.currency,orderId:id});}}
